...
export default function GlassColorsTab() {
  ...
  const handleSave = async (e) => {
    ...
    const url = editingColor
      ? `${API_URL}/glass-colors/${editingColor.id}`
      : `${API_URL}/glass-colors`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_URL}/glass-colors/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md transition">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-200/40 flex justify-center items-center z-50">
